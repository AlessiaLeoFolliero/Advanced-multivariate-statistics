...
In this project we are going to analyze some aspects of Italian region to see if we can end up with something
...
First we need to load all the dataset that we need
...
Now we merge the dataset to create our own dataset.
...
```
We delete some variables and introduce the correlation matrix
...
Data_sen$Pers_risch_pov
#Bartlett's test.
#Null hypothesis is to test if the variables are ortogonal, so that this means that they are correlated enought to differ from the identity matrix
...
summary(PCA)
```
Each component explains a percentage of the total variance in the dataset. The first component is able itself to explain 85 % of the total variability, and the seconf 8% of the total variance.
...
The loadings goes from (-1 to 1)
A high absolute value means that the variable strongly influence the component, while if the value is close to zero the variable does not influence the component that much.
...
```
We can see that most of the variables have a negative value in the first component and that the only value which is close to zero is Reseach expenditure proportionate to GDP which is actually a little higher in the second component.
...
The higher the distance between the variable and the origin the better that variable is represented. (in fact what we have already seen about R.S_GDP is also explained here)
The variables that are on the leften side of the biplot are negatively correlated with the first principal component
...
```
The optimal selected model is EEV: that is two components are ellipsoidal with equal colume and shape.
...
Positive correlated object are close together on the same side of the plot, as we can see on the leften side of the plot.
...
To check if there is really a difference between those groups I am going to perform a Manova:
it is the multivariate analysis of the variance
